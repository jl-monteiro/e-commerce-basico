      ...
      {produtos.map((produto) => (
        <div key={produto.id} className="bg-white p-4 rounded-lg shadow-md">
          <Link to={`/produto/${produto.id}`}>
            <img src={produto.imagem_prod} alt={produto.nome_prod} className="w-full h-48 object-cover rounded-lg mb-4" />
            <div>
              <h1 className="text-lg font-bold">{produto.nome_prod}</h1>
              <h2 className="text-gray-600">{produto.descricao_prod}</h2>
              <span className="text-indigo-500">{toBRL(produto.preco_prod)}</span>
            </div>
          </Link>
        </div>
      ))}
